...
handlers._users.POST = (data, callback) => {
  ...
  let name = typeof (data.payload.firstname) == 'string' && data.payload.firstname.trim().length > 0 ? data.payload.firstname : false;
  let lname = typeof (data.payload.lastname) == 'string' && data.payload.lastname.trim().length > 0 ? data.payload.lastname : false;
  let phone = typeof (data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone : false;
  let password = typeof (data.payload.password) == 'string' && data.payload.password.trim().length > 1 ? data.payload.password : false;
  let tosAgreement = typeof (data.payload.tosAgreement) == 'boolean' ? true : false;


  if (name && lname && phone && password && tosAgreement) {
      ...
      if (err) {
        ...
        if (hashedPwd) {
          ...
          _data.create('users', phone, usrObj, (err) => {
            if (!err) {
              callback(200, { 'message': 'data saved successfully' });
            } else {
              callback(500, { 'Error': err })
            }
          })
        } else {
          callback(500, { 'Error': 'Couldn\'t hash users password.' })
        }
      } else {
...
handlers._tokens.POST = (data, callback) => {
  let phone = typeof(data.payload.phone) == 'string' && data.payload.phone.trim().length == 10 ? data.payload.phone : false;
  let password = typeof(data.payload.phone) == 'string' && data.payload.password.trim().length > 0 ? data.payload.password : false;
  ...
  if (phone && password) {
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        ...
        if (hashedPassword == helpers.parseJsonToObject(userData).hashedPassword) {
          //create token object
...
handlers._tokens.GET = (data, callback) => {
  let id = typeof (data.queryStringObject.token) == 'string' && data.queryStringObject.token.trim().length == 20 ? data.queryStringObject.token : false;
  if (id) {
    ...
    _data.read('tokens',id,(err,tokenData) => {
      if(!err && tokenData){
        if(helpers.parseJsonToObject(tokenData).expires > Date.now()){
          ...
          _data.update('tokens',id,helpers.parseJsonToObject(tokenData),(err) => {
            if(!err){
              callback(200,helpers.parseJsonToObject(tokenData));
            } else {
  ...
  if(id){
    _data.read('checks',id,(err,checkData) => {
      if(!err && checkData){
        ...
        handlers._tokens.verifyToken(token,helpers.parseJsonToObject(checkData).userPhone,(tokenIsValid) => {
          if(tokenIsValid){
